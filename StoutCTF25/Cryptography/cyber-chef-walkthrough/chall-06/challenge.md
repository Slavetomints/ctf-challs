...
The way that ROT works is that is shifts letters one to the side, such as that `a` becomes `b`, and so on and so forth. The Ceaser Cipher works with a shift of three characters. `a` becomes `d`, `b` becomes `e`, and etc. 
...
If we know what the original shift was, we can subtract that from 26, which is why if you shift something by 13, all out need to d ois shift it by 13 again.
